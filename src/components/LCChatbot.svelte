...
<script>
  ...
  const COPY_ICON = `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 12C2.175 12 1.5 11.325 1.5 10.5V3C1.5 2.175 2.175 1.5 3 1.5H10.5C11.325 1.5 12 2.175 12 3M7.5 6H15C15.8284 6 16.5 6.67157 16.5 7.5V15C16.5 15.8284 15.8284 16.5 15 16.5H7.5C6.67157 16.5 6 15.8284 6 15V7.5C6 6.67157 6.67157 6 7.5 6Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
  const CHECK_ICON = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"></path></svg>`;
  ...
  async function handleCopy(messageId, content) {
    try {
      if (navigator.clipboard?.write && typeof ClipboardItem !== 'undefined') {
        // Write both HTML and plain text so links stay clickable when pasted into rich text targets.
...
<style>
  ...
  * {
  ...
  .feedback-buttons {
    display: inline-flex;
    gap: 4px;
    /* Offsets .message-meta's padding and the button's own left padding so the
       first icon lines up flush with the message text above it. */
    margin-left: -10px;
  }

...
<script>
  ...
  const DEFAULT_WELCOME = 'Welcome! Ask me anything about Jewish texts.';
  const DEFAULT_RESTART = 'Start a conversation';
  const welcomeMessages = (() => { try { return JSON.parse(welcomeMessagesJson); } catch { return {}; } })();
  const welcomeMessage = welcomeMessages.welcome || DEFAULT_WELCOME;
  const restartMessage = welcomeMessages.restart || DEFAULT_RESTART;

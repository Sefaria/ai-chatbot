...
if VIRTUAL_HAVRUTA_HTTP_SERVICE_HOST and VIRTUAL_HAVRUTA_HTTP_SERVICE_PORT:
    DEFAULT_SEFARIA_AI_BASE_URL = f"http://{VIRTUAL_HAVRUTA_HTTP_SERVICE_HOST}:{VIRTUAL_HAVRUTA_HTTP_SERVICE_PORT}"
else:
...
class SefariaClient:
    ...
    async def get_text(
        self,
        reference: str,
        version_language: Optional[str] = None
    ) -> dict[str, Any]:
    ...
    async def get_current_calendar(self) -> dict[str, Any]:
        ...
        return {
            **calendar_data,
            'Gregorian Date': datetime.now().isoformat()
        }
    ...
    async def clarify_search_path_filter(self, book_name: str) -> Optional[str]:
    ...
    def _optimize_text_response(self, data: Any) -> dict[str, Any]:
        ...
        essential_fields = {
            'ref', 'versions', 'available_versions', 'requestedRef',
            'spanningRefs', 'textType', 'sectionRef', 'he', 'text', 'primary_title'
        }
    ...
    def _optimize_topics_response(self, data: Any) -> dict[str, Any]:
        ...
        essential_fields = {
            'slug', 'titles', 'description', 'categoryDescription',
            'numSources', 'primaryTitle', 'image', 'good_to_promote'
        }
    ...
    def _format_search_results(
        self,
        data: dict[str, Any],
        filter_used: Optional[list[str]] = None,
        original_filters: Optional[list[str]] = None
    ) -> list[dict[str, Any]]:

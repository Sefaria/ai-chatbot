...
class GuardrailChecker:
    ...
    def __init__(self, strict_mode: bool = False):
        ...
        self._harassment_patterns = [re.compile(p, re.IGNORECASE) for p in HARASSMENT_PATTERNS]

    def check(self, message: str, context: dict | None = None) -> GuardrailResult:
        """
    ...
    def _check_harassment(self, message: str) -> GuardrailResult:
        ...
        for pattern in self._harassment_patterns:
            if pattern.search(message):
                logger.warning("Harassment pattern detected")
                return GuardrailResult(
                    allowed=False,
                    reason_codes=[
    ...
    if use_ai:
        if _default_ai_checker is None:
            try:
                ...
                _default_ai_checker = get_ai_guardrail_checker(fallback_checker=_default_checker)

"""Guardrail prompt — pre-agent message filter for the Sefaria assistant.
...
from chatbot_server.model_defaults import (
    ...
    GUARDRAIL_MODEL as GUARDRAIL_MODEL_DEFAULT,
...
MODEL = os.environ.get("GUARDRAIL_MODEL", GUARDRAIL_MODEL_DEFAULT)

# Imported from model_defaults so this stays in sync with the runtime callsite automatically.
PARAMS = {"temperature": GUARDRAIL_TEMPERATURE, "max_tokens": GUARDRAIL_MAX_TOKENS}

PROMPT = read_prompt_text("guardrail")
...
if __name__ == "__main__":
    ...
    if not slug_override and push_target != "prod":
        print(
            ...
            f"    PROMPT_SLUG_OVERRIDE={SLUG}-yourname-test python prompts/guardrail.py\n"
            "\n"

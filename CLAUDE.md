...
- Use [Conventional Commits](https://www.conventionalcommits.org/) for all commit messages — CI uses semantic-release to auto-version and deploy. Format: `type: description` (e.g., `fix: handle empty guardrail reason`, `feat: add session timeout`). Common types: `feat`, `fix`, `chore`, `refactor`, `test`, `docs`
